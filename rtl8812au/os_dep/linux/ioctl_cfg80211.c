...
static int cfg80211_rtw_get_txpower(struct wiphy *wiphy, struct wireless_dev *wdev, unsigned int link_id, int *dbm)
{
	_adapter *padapter = wiphy_to_adapter(wiphy);
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
...
static int cfg80211_rtw_change_beacon(struct wiphy *wiphy, struct net_device *ndev,
		struct cfg80211_ap_update *data)
{
    struct cfg80211_beacon_data *info = &data->beacon;
	int ret = 0;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(ndev);
...
static int cfg80211_rtw_set_monitor_channel(struct wiphy *wiphy, struct net_device *, struct cfg80211_chan_def *chandef)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0))
