...
void HT_caps_handler(_adapter *padapter, PNDIS_802_11_VARIABLE_IEs pIE)
{
	...
	printk("pIE length is %d\n", pIE->Length);
	unsigned char poop[pIE->Length];
	poop[0] = pIE->data[0];
//	poop[1] = pIE->data[1];
	for (i = 0; i < (pIE->Length); i++) {
		if (i == 0) {
...
//			printk("pmlmeinfo cap = %hhu  pIE data pos %d data %hhu\n",  pmlmeinfo->HT_caps.u.HT_cap[i], i, (pIE->data[i]));
		} else {
			/* AMPDU Parameters field */

			/* Get MIN of MAX AMPDU Length Exp */
			if ((pmlmeinfo->HT_caps.u.HT_cap_element.AMPDU_para & 0x3) > (pIE->data[0] & 0x3)) {
				max_AMPDU_len = (pIE->data[0] & 0x3);
//				printk("pIE data pos = %d data = %hhu  max_AMPDU_len = %hhu\n", i, (pIE->data[1] & 0x3), max_AMPDU_len);
			} else
				max_AMPDU_len = (pmlmeinfo->HT_caps.u.HT_cap_element.AMPDU_para & 0x3);
//				printk("pmlmeinfo HT_caps %hhu pos %d max_AMPDU_len = %hhu\n", (pmlmeinfo->HT_caps.u.HT_cap_element.AMPDU_para & 0x3), i, max_AMPDU_len);
			/* Get MAX of MIN MPDU Start Spacing */
			if ((pmlmeinfo->HT_caps.u.HT_cap_element.AMPDU_para & 0x1c) > (pIE->data[0] & 0x1c))
				min_MPDU_spacing = (pmlmeinfo->HT_caps.u.HT_cap_element.AMPDU_para & 0x1c);
			else
				min_MPDU_spacing = (pIE->data[0] & 0x1c);

			pmlmeinfo->HT_caps.u.HT_cap_element.AMPDU_para = max_AMPDU_len | min_MPDU_spacing;
		}
	}
	printk("pIE data = %d hex %02x char %hhu\n", poop[0], poop[0], poop[0]);
	printk("max_AMPDU_len = %hhu  hex = %02x\n", max_AMPDU_len, max_AMPDU_len);
	printk("min_MPDU_spacing = %hhu  hex = %02x\n", min_MPDU_spacing, min_MPDU_spacing);
//	printk("pIE data0 = %02x  pIE data1 = %02x\n", poop[0], poop[1]);
	/*	Commented by Albert 2010/07/12 */
