...
//	printk("usb_dvobj_deinit %s %d\n", __func__, &usb_intf->dev.kobj.kref.refcount);
//	usb_get_dev(interface_to_usbdev(usb_intf));
//	unsigned int refcount = module_refcount(THIS_MODULE);
//	printk("usb_dvobj_deinit refcount = %u\n", refcount);
...
//	unsigned int refcount = module_refcount(THIS_MODULE);
//	printk("usb_intf_stop refcount = %u\n", refcount);

//	printk("usb_intf_stop \n");
}
...
void autosuspend_enter(_adapter *padapter)
{
	...
	if (rf_off == pwrpriv->change_rfpwrstate) {
		...
		if (1 == pwrpriv->autopm_cnt) {
			...
			printk("0!=pwrpriv->autopm_cnt[%d]   didn't usb_autopm_put_interface\n", pwrpriv->autopm_cnt);
...
int autoresume_enter(_adapter *padapter)
{
	...
	if (rf_off == pwrpriv->rf_pwrstate) {
		...
		if (0 == pwrpriv->autopm_cnt) {
			...
			RTW_INFO("...pm_usage_cnt(%d).....\n", dvobj->pusbintf->pm_usage_cnt);
#endif
//			usb_autopm_get_interface(dvobj->pusbintf);
			pwrpriv->autopm_cnt++;
		} else
...
#endif
//printk("rtw_usb_primary_adapter_deinit\n");
}
...
//	struct devive *dev = dvobj_to_dev(dvobj);
//	kref_init(&dvobj->kref);
...
//	unsigned int refcount = module_refcount(THIS_MODULE);
//	printk("rtw_dev_remove 1 refcount = %u\n", refcount);
...
//	printk("dereferencing rtl8812 driver\n");	
//	usb_put_dev(dvobj->pusbdev);
//	printk("rtw_dev_remove %s\n", &pusb_intf->dev.kobj.kref.refcount);
//	refcount = module_refcount(THIS_MODULE);
//	printk("rtw_dev_remove 2 refcount = %u\n", refcount);
//	int *test = (int*)&pusb_intf->dev.kobj.kref.refcount;
//	printk("ref count dev_remove %d\n", test);
//	printk("stopping rtl8812 driver\n");

	usb_dvobj_deinit(pusb_intf);

//	printk("deinit of rtl8812 driver complete\n");
